...
export default function PackOpenFlow({ pack, packName, onClose }) {
  ...
  const [slideDir, setSlideDir] = useState('right'); // which side new card enters from
  const [cardKey, setCardKey]   = useState(0);        // forces re-mount for animation

  // swipe tracking
  const swipeRef = useRef({ active: false, x0: 0, y0: 0, dx: 0, dy: 0 });
  const [liveDx, setLiveDx]     = useState(0);        // card follows finger
  ...
  const onEnd = () => {
    ...
    if (phase === 'stack') {
      if (dx < -55 && cardIdx < total - 1) {
        setSlideDir('right');
        setCardIdx(i => i + 1);
        setCardKey(k => k + 1);
      } else if (dx > 55 && cardIdx > 0) {
        setSlideDir('left');
        setCardIdx(i => i - 1);
        setCardKey(k => k + 1);
      }
  ...
  const ghosts = [];
  for (let i = 1; i <= Math.min(3, total - cardIdx - 1); i++) {
    ghosts.push(i);
  }
  ...
  /* ── tear pieces: same pack-body, clipped top/bottom ── */
  const TearHalf = ({ top }) => (
    <div
  ...
  return (
    ...
    >
      {/* ── EXPANDING / READY ── */}
      {(phase === 'expanding' || phase === 'ready') && (
        <div className={`pof-pack-stage pof-${phase}`}>
          <div
            className="pof-pack-wrap"
            style={phase === 'ready' && liveDx !== 0
      ...
      {/* close button — always available except mid-tear */}
      {phase !== 'tearing' && (
